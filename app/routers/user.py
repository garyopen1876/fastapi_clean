...
router = APIRouter(dependencies=[Depends(token_authentication)])


@router.post("/", status_code=status.HTTP_201_CREATED, summary="User create")
def user_create(username: str = Body(...), password: str = Body(...), email: str = Body(...)):
    return utils.user_create(username, password, email)


@router.delete("/", status_code=status.HTTP_200_OK, summary="User delete")
def user_delete(user_id: int):
    return utils.user_delete(user_id)


@router.put("/reset_password", status_code=status.HTTP_200_OK, summary="User reset password")
